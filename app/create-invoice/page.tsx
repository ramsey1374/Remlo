  ...
  if (!isConnected) {
    return (
      <div className="min-h-screen bg-[#0d0d14] flex items-center justify-center px-6">
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full bg-indigo-500/5 blur-[120px] pointer-events-none" />
        <div className="relative z-10 text-center max-w-sm w-full">
          <div className="flex items-center justify-center mb-10"><RemloLogo size={120} /></div>
          <h1 className="text-white text-3xl font-bold tracking-tight mb-3">Get paid in USDC</h1>
          <p className="text-white/40 text-sm leading-relaxed mb-10">
            Create invoice links. Your payer can pay from any chain — you always receive USDC on Arc Testnet.
          </p>
          <div className="flex flex-col gap-3 mb-10 text-left">
            {[
              { icon: "⚡", text: "Automatic chain routing" },
              { icon: "🔵", text: "Always settles to Arc Testnet" },
              { icon: "🔗", text: "Shareable payment links" },
            ].map((f) => (
              <div key={f.text} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.06]">
                <span>{f.icon}</span>
